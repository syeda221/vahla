...
<div class="content-wrapper">
    <div class="page-header">
        <h3 class="page-title">Edit Direct Delivery Challan</h3>
    </div>
    <div class="row">
                    ...
                    <form action="{{ route('direct-dc.update', $dc->id) }}" method="POST">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-8 form-group">
                                <label>Remarks</label>
                                <input type="text" name="remarks" class="form-control" value="{{ $dc->remarks }}">
                            </div>
                        </div>
...
$(document).ready(function() {
    $('#itemsTable tbody tr').each(function() {
        ...
        if (price === 0) {
            var val = tr.find('.product-select').val();
            if (val) {
                var parts = val.split('|variant|');
                var realId = parts[0];
                $.get('{{ route("get-price") }}', { product_id: realId }, function(res) {
                    if (res && res.retail_price !== undefined) {
                        var p = res.retail_price;
                        var vdEncoded = tr.find('.variant-data-hidden').val();
                        if (vdEncoded) {
                            try {
                                var vData;
                                try {
                                    vData = JSON.parse(atob(vdEncoded));
                                } catch(e) {
                                    vData = JSON.parse(vdEncoded);
                                }
                                if (vData.sale_price) p = vData.sale_price;
                            } catch(e) {}
                        }
                        tr.find('.price-input').val(p);
                        calculateRow(tr);
                    }
                });
            }
